...
import { GET_USER_PROFILE } from '../modules/authentication/login/login.graphql';
...
export class AuthService {
  ...
  objectPerms: any;
  perms: any;
  taesaRegistrationNumber$ = new BehaviorSubject<string | undefined>(undefined);
  photoUrl$ = new BehaviorSubject<string | undefined>(undefined);
  principal: any;
        ...
        .pipe(
          ...
          map((data) => {
            if (data) {
              ...
              result = result.getUserProfileAndRole;

              console.log(
                result.data.userProfile.profileType === UserTypesEnum.DOCTOR
              );
              console.log(UserTypesEnum.DOCTOR);
              console.log(result.data.userProfile.profileType);

              if (result.response.status) {
                this.storageService.setItem('profile', result.data.userProfile);
                if (
                  result.data.userProfile.profileType !== UserTypesEnum.DOCTOR
                ) {
                  const finalArray = result.data.userRoles.rolePermissions?.map(
                    (obj: any) => {
                      return obj?.permissionCode;
                    }
                  );

                  this.permissionService.loadPermissions(finalArray);
